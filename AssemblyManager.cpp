                ...
                for ( int x = -5; x <= 5; ++x ) {
                    for ( int y = -5; y <= 5; ++y ) {
                        ...
                        if ( !(x == 0 && y == 0) &&
                            ...
                            inventory.map_veins_[WalkPosition(TilePosition( centralize_x, centralize_y )).x][WalkPosition( TilePosition( centralize_x, centralize_y ) ).y] < 150 && // don't wall off please. Wide berth around blue veins.
