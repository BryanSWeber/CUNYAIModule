...
void Stored_Unit::updateStoredUnit(const Unit &unit){
    ...
    if (type_ != unit->getType()) {
        ...
        bool retreating_undetected= ( phase_ != "Retreating" || phase_ != "Attacking" || burrowed_ || !detected_ ); // detected doesn't work for personal units, only enemy units.
        ...
        if(unit->getPlayer() == Broodwar->self()) ma_future_fap_value_ = retreating_undetected ? stock_value_ : static_cast<int>(weight * ma_future_fap_value_ + (1.0 - weight) * future_fap_value_);
