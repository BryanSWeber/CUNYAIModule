...
bool CUNYAIModule::checkSuperiorFAPForecast2(const Unit &u, const Unit_Inventory &ui, const Unit_Inventory &ei) {
    bool unit_alive = ui.unit_inventory_.find(u)!= ui.unit_inventory_.end() && Stored_Unit::unitAliveinFuture(ui.unit_inventory_.at(u), 24);
    return  ( ( (ui.stock_fighting_total_ - ui.moving_average_fap_stock_) * ei.stock_fighting_total_ <= (ei.stock_fighting_total_ - ei.moving_average_fap_stock_) * ui.stock_fighting_total_) && unit_alive) || // Proportional win. fixed division by crossmultiplying. Added unitalive in future so the bot is more reasonable in combat situations.
            //(ui.moving_average_fap_stock_ - ui.future_fap_stock_) < (ei.moving_average_fap_stock_ - ei.future_fap_stock_) || //Win by damage.
             ui.moving_average_fap_stock_ > ei.moving_average_fap_stock_; //Antipcipated victory.
}