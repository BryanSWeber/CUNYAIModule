...
bool MeatAIModule::Tech_Begin(Unit building, const Unit_Inventory &ui) {
    ...
    busy += Check_N_Research( TechTypes::Lurker_Aspect, building, upgrade_bool && (Count_Units( UnitTypes::Zerg_Lair, friendly_inventory ) > 0 || Count_Units( UnitTypes::Zerg_Hive, friendly_inventory ) > 0) && Count_Units( UnitTypes::Zerg_Hydralisk_Den, friendly_inventory ) > 0 );
    ...
    busy += Check_N_Upgrade(UpgradeTypes::Adrenal_Glands, building, upgrade_bool && Count_Units(UnitTypes::Zerg_Hive, friendly_inventory) > 0);
    busy += Check_N_Upgrade(UpgradeTypes::Anabolic_Synthesis, building, upgrade_bool && Count_Units(UnitTypes::Zerg_Ultralisk_Cavern, friendly_inventory) > 0);
    busy += Check_N_Upgrade(UpgradeTypes::Chitinous_Plating, building, upgrade_bool && Count_Units(UnitTypes::Zerg_Ultralisk_Cavern, friendly_inventory) > 0);
