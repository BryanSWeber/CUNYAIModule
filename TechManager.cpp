...
void MeatAIModule::Tech_Begin(Unit building, const Unit_Inventory &ui) {
    ...
    Check_N_Upgrade( UpgradeTypes::Zerg_Flyer_Attacks, building, tech_starved && Count_Units( UnitTypes::Zerg_Spire, friendly_inventory ) > 0 && (ui.stock_fliers_ > ui.stock_ground_units_ || BWAPI::Broodwar->self()->getUpgradeLevel( UpgradeTypes::Zerg_Carapace ) == 3 ));
    Check_N_Upgrade( UpgradeTypes::Zerg_Flyer_Carapace, building, tech_starved && Count_Units( UnitTypes::Zerg_Spire, friendly_inventory ) > 0 && (ui.stock_fliers_ > ui.stock_ground_units_ || BWAPI::Broodwar->self()->getUpgradeLevel( UpgradeTypes::Zerg_Carapace ) == 3 ));
    ...
    Check_N_Upgrade( UpgradeTypes::Antennae, building, tech_starved && (Count_Units( UnitTypes::Zerg_Lair, friendly_inventory ) > 0 || Count_Units( UnitTypes::Zerg_Hive, friendly_inventory ) > 0) ); //don't need lair if we have a hive. This upgrade is terrible, thus last.
