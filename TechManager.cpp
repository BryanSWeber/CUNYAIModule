...
bool CUNYAIModule::Tech_Begin(Unit building, const Unit_Inventory &ui, const Inventory &inv) {
    ...
    busy += Check_N_Upgrade( UpgradeTypes::Metabolic_Boost, building, upgrade_bool && Stock_Units( UnitTypes::Zerg_Zergling, friendly_inventory) > 0 );
    busy += Check_N_Research( TechTypes::Lurker_Aspect, building, upgrade_bool && (Count_Units( UnitTypes::Zerg_Lair, inv ) > 0 || Count_Units( UnitTypes::Zerg_Hive, inv ) > 0) && Count_Units( UnitTypes::Zerg_Hydralisk_Den, inv ) > 0 );
    busy += Check_N_Upgrade( UpgradeTypes::Grooved_Spines, building, upgrade_bool && (Stock_Units( UnitTypes::Zerg_Hydralisk, friendly_inventory) > Stock_Units( UnitTypes::Zerg_Zergling, friendly_inventory) || BWAPI::Broodwar->self()->getUpgradeLevel( UpgradeTypes::Zerg_Melee_Attacks ) == 3) );
    busy += Check_N_Upgrade( UpgradeTypes::Muscular_Augments, building, upgrade_bool && (Stock_Units( UnitTypes::Zerg_Hydralisk, friendly_inventory) > Stock_Units( UnitTypes::Zerg_Zergling, friendly_inventory) || BWAPI::Broodwar->self()->getUpgradeLevel( UpgradeTypes::Zerg_Melee_Attacks ) == 3 ) );
    busy += Check_N_Upgrade(UpgradeTypes::Pneumatized_Carapace, building, upgrade_bool && (Count_Units(UnitTypes::Zerg_Lair, inv) > 0 || Count_Units(UnitTypes::Zerg_Hive, inv) > 0));
    busy += Check_N_Upgrade(UpgradeTypes::Adrenal_Glands, building, upgrade_bool && Count_Units(UnitTypes::Zerg_Hive, inv) > 0);
    busy += Check_N_Upgrade(UpgradeTypes::Anabolic_Synthesis, building, upgrade_bool && Count_Units(UnitTypes::Zerg_Ultralisk_Cavern, inv) > 0);
    busy += Check_N_Upgrade(UpgradeTypes::Chitinous_Plating, building, upgrade_bool && Count_Units(UnitTypes::Zerg_Ultralisk_Cavern, inv) > 0);
    ...
    busy += Check_N_Upgrade( UpgradeTypes::Zerg_Missile_Attacks, building, upgrade_bool && (Stock_Units( UnitTypes::Zerg_Hydralisk, friendly_inventory ) > Stock_Units( UnitTypes::Zerg_Zergling, friendly_inventory ) || BWAPI::Broodwar->self()->getUpgradeLevel( UpgradeTypes::Zerg_Melee_Attacks ) == 3) );
    busy += Check_N_Upgrade(UpgradeTypes::Zerg_Flyer_Attacks, building, upgrade_bool && Count_Units(UnitTypes::Zerg_Spire, inv) > 0 && (ui.stock_fliers_ > ui.stock_ground_units_ || BWAPI::Broodwar->self()->getUpgradeLevel(UpgradeTypes::Zerg_Carapace) == 3));
    busy += Check_N_Upgrade(UpgradeTypes::Zerg_Flyer_Carapace, building, upgrade_bool && Count_Units(UnitTypes::Zerg_Spire, inv) > 0 && (ui.stock_fliers_ > ui.stock_ground_units_ || BWAPI::Broodwar->self()->getUpgradeLevel(UpgradeTypes::Zerg_Carapace) == 3));
