...
// Personally made functions:
  ...
  void Check_N_Grow( BWAPI::UnitType unittype, BWAPI::Unit larva, bool extra_critera );
  ...
  // Checks if it is a fighting unit. UAB actually missed some logic in here.
  bool IsFightingUnit( BWAPI::Unit unit );
  // Draws a line if diagnostic mode is TRUE.
  void Diagnostic_Line( BWAPI::Position s_pos, BWAPI::Position f_pos, BWAPI::Color col );
  // Outlines the case where you cannot attack their type (air/ground/cloaked), while they can attack you.
  bool MeatAIModule::Futile_Fight( BWAPI::Unit unit, BWAPI::Unit enemy );
  // Outlines the case where you can attack their type (air/ground/cloaked)
  bool MeatAIModule::Can_Fight( BWAPI::Unit unit, BWAPI::Unit enemy );

  // Basic retreat logic, range 350.
  void Retreat_Logic( BWAPI::Unit unit, BWAPI::Unit enemy, BWAPI::Color color );
  // Tells the unit to fight. If it can attack both air and ground.
  void Combat_Logic( BWAPI::Unit unit, BWAPI::Color color );  
  ...
  void Brownian_Stutter( BWAPI::Unit unit, double n );

  //gathers win history. updates one of the 6 primary values of interest, matched on race. 
