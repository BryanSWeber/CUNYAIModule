...
void MeatAIModule::onStart()
{
	...
	delta = 0.25; //gas starved parameter. Triggers state if: ln_gas/(ln_min + ln_gas) < delta;  Higher is more gas.
	phi = 0.55; //army starved parameter. Triggers state if: army_heurisitic/(army_heuristic+econ_heuristic) < phi; Higher is more army.
...
void MeatAIModule::onFrame()
{
  ...
  //Define important variables.
	...
	bool builder_exists = Broodwar->self()->incompleteUnitCount(UnitTypes::Buildings) > 0;
  ...
  Broodwar->drawTextScreen(10, 70, "Builders: %d", builder_exists);  //
  ...
  Broodwar->drawTextScreen(100, 20, "Proportion Gas: %f", ln_gas / (ln_min + ln_gas) );
  ...
  Broodwar->drawTextScreen(250, 40, "Ling Count: %d", ling_count);  // 

  Broodwar->drawTextScreen(100, 50, "Performance:");  // 
  Broodwar->drawTextScreen(100, 60, "APM: %d", Broodwar->getAPM() );  // 
  Broodwar->drawTextScreen(100, 70, "APF: %f", ( Broodwar->getAPM()/60 ) / Broodwar->getAverageFPS() );  // 
  Broodwar->drawTextScreen(100, 80, "FPS: %f", Broodwar->getAverageFPS() );  // 
  Broodwar->drawTextScreen(100, 90, "Frames of Latency: %d", Broodwar->getLatencyFrames() );  // 
  ...
  for (auto &u : Broodwar->self()->getUnits())
  {
      ...
      // Mining loop if our worker is idle (includes returning $$$) or not moving while gathering gas, we (re-) evaluate what they should be mining.  Original script uses isIdle() only.
		if ( u->isIdle() || !u->isMoving() )
		{
			...
			// Idle worker then Harvest from the nearest mineral patch or gas refinery, depending on need.
				...
				else if (extractor && gas_starved && gas_workers < 3) // If you have an extractor and you need gas then collect some gas. Queue this command, so complete your current task first.
				{
	...
	// Begin a loop for creating buildings of the appropriate type. Note builder resets every few frames.
		if ((u->isIdle() || (u->isGatheringMinerals() && !u->isCarryingMinerals())) && (t + 100 < Broodwar->getFrameCount()) && builder_exists == false)
		{ //only get those that are gathering minerals, but not carrying them. This always irked me. Don't send a builder if you have a builder in this loop.
			  ...
			  //Army Loop
			  if ((Broodwar->self()->minerals() >= UnitTypes::Zerg_Spawning_Pool.mineralPrice()) && army_starved && !pool)
			  ...
			  //Expo Loop
			  if ((Broodwar->self()->minerals() >= UnitTypes::Zerg_Hatchery.mineralPrice() * 1.5))
			  { //if we are floating too much money, get an expo.
				  ...
				  //t += UnitTypes::Zerg_Hatchery.buildTime();
				  builder_exists == true;
			  }
