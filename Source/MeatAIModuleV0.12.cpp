...
// workers are too happy long distance mining.
// add an inablity to attack the target to the retreat command.
...
void MeatAIModule::onStart()
{
    ...
    gamma = 0.63; //supply starved parameter. Triggers state if: ln_supply_remain/ln_supply_total < gamma; Some good indicators that this is reasonable: ln(4)/ln(9) is around 0.63, so we will build our first overlord at 7/9 supply. ln(18)/ln(100) is also around 0.63, so we will have a nice buffer for midgame.
	...
	alpha_army = 0.17300; // army starved parameter.  
    alpha_vis =  0.82025; // vision starved parameter. Note the scale for vision, vision comes in groups of thousands. 
    alpha_econ = 0.00600; // econ starved parameter. 
    alpha_tech = 0.00075; // tech starved parameter.
...
{ // Called once every game frame
  ...
  int min_fields = 1;
    ...
    Unitset resource = BWAPI::Broodwar->getUnitsInRadius( home_x, home_y, 99999, Filter::IsMineralField); // get any mineral field

    if ( !resource.empty() ) { // check if the minerals exist

        for (auto r = resource.begin(); r != resource.end(); ++r) { //for each resource

            Unitset mybases = Broodwar->getUnitsInRadius( (*r)->getPosition(), 250, Filter::IsResourceDepot && Filter::IsOwned); // is there a mining base there?

            if ( !mybases.empty() ) { // check if there is a base nearby
                for (auto base = mybases.begin(); base != mybases.end(); ++base) {
                    Broodwar->drawCircleMap( (*base)->getPosition(), 250, Colors::Green);
                } // if nearby base, draw a pretty picture.
                        (*min_fields_ptr)++; // only increment mineral count if there exist nearby bases.
            }
        }// closure for each resource.
  ...
  supply_starved = Stock_Supply(UnitTypes::Zerg_Overlord) +
      ...
      Stock_Supply(UnitTypes::Zerg_Lair) +
      Stock_Supply(UnitTypes::Zerg_Hive) -
      Broodwar->self()->supplyUsed() + 5 < 0 || //If you have eaten up  your supply you are supply blocked
 ...
 ; // If you supply is negative, then you're supply blocked. no fancy ln 0 errors here.
  // Cobb-Douglas 
  ...
  for ( auto &u : Broodwar->self()->getUnits() )
  {
    ...
    //Combat loop. Has some sophistication at this time. Moved to onUnitDiscover to avoid APM drain.
    // make retreat/attack decision.  Only retreat if your army is not up to snuff. Only combat units retreat. Only retreat if the enemy is near.
    Unitset e_near_set = Broodwar->getUnitsInRadius( u->getPosition(), 1024, Filter::IsEnemy);

    if (!e_near_set.empty() && army_starved) { //only consider retreat if army starved and there is an enemy near
              for (auto e = e_near_set.begin(); e != e_near_set.end() ; ++e) {
                  if (MeatAI::UnitUtil::IsCombatUnit(*e)) {

                      Broodwar->drawCircleMap((*e)->getPosition(), 300, Colors::Red); // not conserving APM well here ... 
                      ...
                      int dist_x = e_pos_x - pos_x;
                      int dist_y = e_pos_y - pos_y;
                      ...
                      if (dist < 350) { // a buffer of 50 pixels beyond my "bare mininimum" safe zone of 300. Run if you're a noncombat unit or army starved.
                          u->move({ pos_x - abs(dist_x) * 350 , pos_y - abs(dist_y) * 350 }); //identify vector between yourself and e.  go 350 pixels away in the quadrant furthest from them.          
                      }
                  } // closure if they are enemy and army starved then retreat
              } // retreat from all enemies in the list
          }// closure: only consider retreat if army starved and there is an enemy.
     else if (!army_starved && !Broodwar->enemy()->getUnits().empty() && MeatAI::UnitUtil::IsCombatUnit(u)) {

              Unit target_caster = u->getClosestUnit(Filter::IsEnemy && (Filter::IsSpellcaster && !Filter::IsAddon && !Filter::IsBuilding) && !Filter::IsCloaked); // don't allin buildings pz. Thanks.  These definitions of "is spellcaster", etc. are terrible.
              Unit target_warrior = u->getClosestUnit(Filter::IsEnemy && Filter::IsAttacking && !Filter::IsCloaked);
              Unit target_other = u->getClosestUnit(Filter::IsEnemy && !Filter::IsCloaked);

              if (target_caster) { //priortize caster units first.
                  u->attack(target_caster);  
              }
              else if ( target_warrior ) { // then combat units.
                  Unit target = u->getClosestUnit(Filter::IsEnemy && !Filter::IsCloaked);
