...
void MeatAIModule::onStart()
{
  ...
  // Hello World!
  Broodwar->sendText("Hello world! This is MeatShieldAI V0.04");
...
void MeatAIModule::onFrame()
{
  ...
  //Define important variables.
	ln_min = log( Broodwar->self()->gatheredMinerals() + 5 );
	ln_gas = log( Broodwar->self()->gatheredGas() + 5 );
	army_econ_ratio = ( log( pow(Broodwar->self()->getUnitScore(), 2) ) / log( pow( exp(ln_min) , 2) + pow( exp(ln_gas) , 2) ) );
  ...
  //Define important states.
	gas_starved = (ln_gas/ln_min < delta) ;  //If your gas gathering history is disproportionately low, then you are gas starved.
  ...
  Broodwar->drawTextScreen(10, 10, "Pool: %d", pool);  // 
  Broodwar->drawTextScreen(10, 20, "Extractor: %d", extractor);  //

  Broodwar->drawTextScreen(100, 0, "Econ State:");  // What is the econ state?
  Broodwar->drawTextScreen(100, 10, "Gas Starved: %d", gas_starved); 
  Broodwar->drawTextScreen(100, 20, "Ln(Gas)/Ln(Min): %f", ln_gas/ln_min ); 
  //Broodwar->drawTextScreen(100, 20, "Worker Count", worker_count = 0);
  ...
  Broodwar->drawTextScreen(250, 20, "Army/Econ: %f", army_econ_ratio);  // 

  Broodwar->drawTextScreen(10, 40, "Performance:");  // 
  Broodwar->drawTextScreen(10, 50, "APM: %d", Broodwar->getAPM() );  // 
  Broodwar->drawTextScreen(10, 60, "FPS: %f", Broodwar->getAverageFPS() );  // 
  Broodwar->drawTextScreen(10, 70, "Frames of Latency: %d", Broodwar->getLatencyFrames() );  // 
  ...
  for (auto &u : Broodwar->self()->getUnits())
  {
	...
	// Begin a loop for creating buildings of the appropriate type. Note builder resets every few frames.
		if ( ( u->isIdle() || ( u->isGatheringMinerals() && !u->isCarryingMinerals() ) ) && (t + 100 < Broodwar->getFrameCount() ) && !builder_exists) 
    ...
    else if ( u->getType().isResourceDepot() ) // A resource depot is a Command Center, Nexus, or Hatchery
    {
      ...
      if ( u->isIdle() && !army_starved && !u->train(u->getType().getRace().getWorker()) )
