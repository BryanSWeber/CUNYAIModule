...
// Note: we are running from the centroid of their army, not the perimiter.
// can get tech'd out. Full tech but still tech starved.  Make it so you're not tech starved at full tech. Ad-Hoc - can build best combat unit if teched out. Worked excellently.
...
void MeatAIModule::onStart()
{
    ...
    alpha_tech = 0.001; // tech starved parameter. This comes in a medium to large scale.
...
{ // Called once every game frame
  ...
  // Return if the game is a replay or is paused
    ...
    Unitset resource = Broodwar->getUnitsInRadius( home_x, home_y, 99999, Filter::IsMineralField ); // get any mineral field you can see

    if ( !resource.empty() ) { // check if the minerals exist
        for ( auto r = resource.begin(); r != resource.end(); ++r ) { //for each mineral

            Broodwar->drawCircleMap( (*r)->getPosition(), 250, Colors::Green ); // Circle each mineral.
            ...
            if ( !mybases.empty() ) { // check if there is a base nearby
                for ( auto base = mybases.begin(); base != mybases.end(); ++base ) {
                    //Broodwar->drawCircleMap( (*base)->getPosition(), 250, Colors::Green ); // if nearby base, draw a pretty picture.
                } 
  ...
  //Vision inventory: Map area could be initialized on startup, since maps do not vary once made.
    ...
    supply_starved = Stock_Supply( UnitTypes::Zerg_Overlord ) +
        Stock_Supply( UnitTypes::Zerg_Hatchery ) +
        Stock_Supply( UnitTypes::Zerg_Lair ) +
        Stock_Supply( UnitTypes::Zerg_Hive ) -
        Broodwar->self()->supplyUsed() + 5 <= 0 || // If you supply is negative, then you're supply blocked. no fancy ln 0 errors here.
        ((ln_supply_remain / ln_supply_total) < gamma &&   //If your supply is disproportionately low, then you are gas starved, unless
            !(ln_supply_total >= log( 200 + 5 ))); // you have not hit your supply limit, in which case you are not supply blocked
    ...
    Broodwar->drawTextScreen( 10, 80, "Lings: %d", (int)MeatAI::UnitUtil::GetAllUnitCount( UnitTypes::Zerg_Zergling ) );  //
    Broodwar->drawTextScreen( 10, 90, "Hydras: %d", (int)MeatAI::UnitUtil::GetAllUnitCount( UnitTypes::Zerg_Hydralisk ) );  //
    Broodwar->drawTextScreen( 10, 100, "Pool: %d", (int)MeatAI::UnitUtil::GetAllUnitCount( UnitTypes::Zerg_Spawning_Pool ) );  // 
    Broodwar->drawTextScreen( 10, 110, "Hydra Den: %d", (int)MeatAI::UnitUtil::GetAllUnitCount( UnitTypes::Zerg_Hydralisk_Den ) );  // 
    Broodwar->drawTextScreen( 10, 120, "Extractor: %d", (int)MeatAI::UnitUtil::GetAllUnitCount( UnitTypes::Zerg_Extractor ) );  //
    Broodwar->drawTextScreen( 10, 130, "Hatches: %d", hatches );  // all hatch types included
    Broodwar->drawTextScreen( 10, 140, "Ovis: %d", (int)MeatAI::UnitUtil::GetAllUnitCount( UnitTypes::Zerg_Overlord ) );  //
 ...
 // Latency frames are the number of frames before commands are processed.
    ...
    for ( auto &u : Broodwar->self()->getUnits() )
    {
            ...
            if ( isIdleEmpty( u ) || u->isCarryingGas() || u->isCarryingMinerals() )
            {
                ...
                // Idle worker then Harvest from the nearest mineral patch or gas refinery, depending on need.
                    bool enough_gas = !gas_starved ||
                        ( (int)MeatAI::UnitUtil::GetAllUnitCount( UnitTypes::Zerg_Extractor ) == 0 ||
                        gas_workers >= 3 * (int)MeatAI::UnitUtil::GetAllUnitCount( UnitTypes::Zerg_Extractor ) );
                    bool enough_minerals = min_workers >= 2 * min_fields;

                    if ( enough_gas || !enough_minerals ) // If we have enough gas, more minerals.
                    {
                        u->gather( u->getClosestUnit( IsMineralField ), true );
                    }
                    else if ( !enough_gas || enough_minerals ) // If we are short on gas, then we should get that.
                    {
        ...
        // Building subloop. Resets every few frames.
            ...
            { //only get those that are idle or gathering minerals, but not carrying them. This always irked me. 
                ...
                //Combat Buildings
                Check_N_Build( UnitTypes::Zerg_Creep_Colony, u, army_starved &&
                    (int) MeatAI::UnitUtil::GetAllUnitCount( UnitTypes::Zerg_Creep_Colony ) == 0 &&
                    ((int)MeatAI::UnitUtil::GetAllUnitCount( UnitTypes::Zerg_Spawning_Pool ) > 0 &&
                    (int)MeatAI::UnitUtil::GetAllUnitCount( UnitTypes::Zerg_Spawning_Pool ) > Broodwar->self()->incompleteUnitCount( UnitTypes::Zerg_Spawning_Pool ) ) ||
                        ((int)MeatAI::UnitUtil::GetAllUnitCount( UnitTypes::Zerg_Evolution_Chamber ) > 0 &&
                        (int)MeatAI::UnitUtil::GetAllUnitCount( UnitTypes::Zerg_Evolution_Chamber ) > Broodwar->self()->incompleteUnitCount( UnitTypes::Zerg_Evolution_Chamber )) );  // a bit tricky, but don't start these unless a building of the approriate type is already done.

                //Expo loop
                if ( (!army_starved || min_fields < 8) ||  // build expos when safe, desparate, or floating minerals
                    (ln_min >= log( UnitTypes::Zerg_Hatchery.mineralPrice() * 1.5 + 1 )) ) {
                    ...
                    Unitset resource_grps = Broodwar->getUnitsInRadius( u->getPosition().x, u->getPosition().y, 99999, Filter::ResourceGroup );
                    double dist = 999999;
                    ...
                    if ( !resource_grps.empty() ) {
                        for ( auto expo = resource_grps.begin(); expo != resource_grps.end(); ++expo ) { // search for closest resource group
                            ...
                            Unitset bases = Broodwar->getUnitsInRadius( expo_x_px_temp, expo_y_px_temp, 800, Filter::IsResourceDepot && Filter::IsOwned );  
                            if ( bases.empty() ) { // check if there are NO bases with 1500p of this resource center.  if there are, don't consider that an expo canidate.
           ...
           //Does a hatchery count as idle if it is researching but has idle larve? Might have to directly select larve at this point.  These should not be ELSE if loops because we can do all of them with multiple larva.
        {
            ...
            if ( (army_starved || tech_starved) &&  //tech starved here is to escape our tech cap, tech can't advance beyond a certain point.
                (int)MeatAI::UnitUtil::GetAllUnitCount( UnitTypes::Zerg_Hydralisk_Den ) > 0 &&
            ...
            if ( ((int)MeatAI::UnitUtil::GetAllUnitCount( UnitTypes::Zerg_Spawning_Pool ) == 0 || !army_starved) &&
                (min_workers < min_fields * 2 || gas_workers >= 3 * (int)MeatAI::UnitUtil::GetAllUnitCount( UnitTypes::Zerg_Extractor )) &&
        ...
        if ( MeatAIModule::isIdleEmpty( u ) &&
           vision_starved &&
           u->getType() == UnitTypes::Zerg_Zergling) //Scout if you're an overlord or a ling. And can move.
        {
            Brownian_Stutter( u, 1 );
        } // If it is a combat unit, then use it to attack the enemy.	
          //if they have cloaked units, get detectors
        if ( MeatAIModule::isIdleEmpty( u ) && 
            u->getType() == UnitTypes::Zerg_Overlord ) {
            Unit c = Broodwar->getClosestUnit( u->getPosition(), Filter::IsEnemy && (Filter::IsBurrowed || Filter::IsCloakable || Filter::IsCloaked) );
            if ( c ) {
                u->move( c->getPosition() );
            }
            else if (vision_starved) {
                Brownian_Stutter( u, 1 );
            }
        }// if they have valid cloakers, bring valid detectors.

        //Upgrade loop:
        if ( u->getType().isBuilding() &&
            MeatAIModule::isIdleEmpty( u ) &&
            (u->canUpgrade() || u->canResearch() || u->canMorph()) &&
            tech_starved ) { // this will need to be revaluated once I buy units that cost gas.
            ...
            Check_N_Build( UnitTypes::Zerg_Hive, u, (tech_derivative > army_derivative) &&
                ...
                u->getType() == UnitTypes::Zerg_Lair ); //If you're tech-starved at this point. build as many hives or lairs as you would like. You don't know what to do with yourself.
    ...
    //Combat loop. Has some sophistication at this time. Makes retreat/attack decision.  Only retreat if your army is not up to snuff. Only combat units retreat. Only retreat if the enemy is near.
        ...
        Unit e = u->getClosestUnit( Filter::IsEnemy && Filter::CanAttack); //Consider combat when there is at least a single enemy.
        if ( e && army_starved && u->canMove()) {
                 *retreat_ptr = true;
                 break;
            }// closure: check for retreat bool.
        if( e && retreat && u->canMove() ){ // Needs there to be an enemy
                    ...
                    if ( dist < 350 ) { // a buffer of 50 pixels beyond my "bare mininimum" safe zone of 300. Run if you're a noncombat unit or army starved.
                        Position retreat_spot = { pos.x - ((dist_x > 0) - (dist_x < 0)) * 350 , pos.y - ((dist_y > 0) - (dist_y < 0)) * 350 }; // the > < functions find the signum, no such function exists in c++!
        ...
        else if ( e && MeatAI::UnitUtil::IsCombatUnit( u ) && u->canMove() && !retreat ) { //Intiatite combat if: we have army, enemies are present, we are combat units and we are not retreating.
            ...
            Unit target_other = u->getClosestUnit( Filter::IsEnemy && !Filter::IsCloaked && Filter::CanAttack );
            if ( target_caster ) { // Neaby casters
                u->attack( target_caster );
            }
            else if ( target_warrior ) { // then nearby combat units.
                u->attack( target_warrior );
            }
            else if ( target_other ) { // then the closest enemy unit we can find.
                u->attack( target_other );
            }
        }// closure Combat loop
